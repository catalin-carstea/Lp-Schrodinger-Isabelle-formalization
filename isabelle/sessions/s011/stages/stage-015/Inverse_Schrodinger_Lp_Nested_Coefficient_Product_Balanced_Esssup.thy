theory Inverse_Schrodinger_Lp_Nested_Coefficient_Product_Balanced_Esssup
  imports
...
section \<open>Balanced actual left nested coefficient endpoint\<close>

context slp_cauchy_local_w1s
begin

theorem slp_nested_coefficient_product_balanced_left_esssup_bound:
...
proof -
  ...
  proof (rule exI[of _ "Ca + Cb"], rule conjI[OF C_positive],
    ...
    let ?whole =
      "\<lambda>z. slp_partial_psi_inverse tau c
        (\<lambda>x. cutoff x *
          slp_dbar_psi_inverse tau c
            (\<lambda>y. coefficient y * multiplier y) x) z"
    let ?low_output =
