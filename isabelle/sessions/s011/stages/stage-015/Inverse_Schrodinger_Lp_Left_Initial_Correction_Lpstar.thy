theory Inverse_Schrodinger_Lp_Left_Initial_Correction_Lpstar
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_015.Inverse_Schrodinger_Lp_Left_Initial_Correction_Power_Loss"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_015.Inverse_Schrodinger_Lp_Nested_Coefficient_Product_High_Endpoint"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Literal left initial-correction secondary norm\<close>

context slp_cauchy_local_w1s
begin

theorem slp_left_neumann_base_lpstar_inverse_sqrt_bound:
...
proof -
  ...
  proof (rule exI[of _ ?C], rule conjI[OF C_positive])
    ...
    proof (intro allI impI)
      ...
      let ?a = "\<lambda>_ :: slp_point. slp_dbar_inverse coefficient c"
      let ?A = "slp_dbar_inverse coefficient"
      let ?DA = "slp_dbar_inverse_gradient coefficient"
      ...
      have zero_dbar:
          "slp_dbar_psi_inverse 0 c coefficient = ?A"
      ...
      have q_zero: "slp_w1p_zero_pair_on p X ?uQ ?DuQ"
        using conjunct1[OF inner_zero_pairs]
        unfolding zero_dbar
        by (simp add: zero_modulation)
      ...
      proof -
        ...
        note first = conjunct1[OF raw[OF inner_premises]]
        show ?thesis
          using first by (simp add: zero_dbar zero_modulation)
      qed
      ...
      have output_presentation:
          "slp_partial_psi_inverse tau c (slp_restrict_field X ?u) = ?T"
        by (simp only: restricted_source_presentation
              slp_left_neumann_base_def)
      ...
      have global:
        ...
        using outer_gain[
          where tau=tau and c=c and f="?u" and Df="?Du" and epsilon=1
            and orientation=SLP_Partial_Inverse,
          OF tau_lower combined_zero]
        by (simp add: slp_partial_psi_inverse_def
              restricted_source_presentation slp_left_neumann_base_def)
      have local_lp:
