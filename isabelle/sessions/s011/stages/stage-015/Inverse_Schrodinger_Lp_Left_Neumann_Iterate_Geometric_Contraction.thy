theory Inverse_Schrodinger_Lp_Left_Neumann_Iterate_Geometric_Contraction
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_015.Inverse_Schrodinger_Lp_Nested_Coefficient_Product_Contraction"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_014.Inverse_Schrodinger_Lp_Exact_Neumann_Iterate_Bridge"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Essential-supremum bounds as almost-everywhere bounds\<close>

lemma slp_ereal_norm_esssup_bound_imp_AE:
  fixes f :: slp_scalar_field and M :: real
  assumes norm_measurable:
      "(\<lambda>x :: slp_point. ereal (Real_Vector_Spaces.norm (f x)))
        \<in> borel_measurable (lborel :: slp_point measure)"
    and esssup_bound:
      "esssup (lborel :: slp_point measure)
          (\<lambda>x :: slp_point. ereal (Real_Vector_Spaces.norm (f x)))
        \<le> ereal M"
  shows "AE x in lborel. Real_Vector_Spaces.norm (f x) \<le> M"
proof -
  have below_esssup:
      "AE x in lborel.
        ereal (Real_Vector_Spaces.norm (f x)) \<le>
          esssup (lborel :: slp_point measure)
            (\<lambda>y :: slp_point.
              ereal (Real_Vector_Spaces.norm (f y)))"
    by (rule esssup_AE)
  show ?thesis
    using below_esssup
    ...
    assume at_x:
      "ereal (Real_Vector_Spaces.norm (f x)) \<le>
        esssup (lborel :: slp_point measure)
          (\<lambda>y :: slp_point.
            ereal (Real_Vector_Spaces.norm (f y)))"
    have "ereal (Real_Vector_Spaces.norm (f x)) \<le> ereal M"
      by (rule order_trans[OF at_x esssup_bound])
    then show "Real_Vector_Spaces.norm (f x) \<le> M"
      by simp
  qed
qed

section \<open>Concrete geometric control of every left Neumann iterate\<close>

context slp_cauchy_local_w1s
begin

theorem slp_left_neumann_iterate_restricted_esssup_geometric:
    ...
    and contraction:
      "\<forall>tau c multiplier N.
        ...
        (\<lambda>z :: slp_point.
          ereal (Real_Vector_Spaces.norm
            (slp_restrict_field X
              (slp_left_neumann_step tau c cutoff coefficient multiplier) z)))
          \<in> borel_measurable (lborel :: slp_point measure)
        \<and>
        esssup (lborel :: slp_point measure)
          (\<lambda>z :: slp_point.
            ereal (Real_Vector_Spaces.norm
              (slp_restrict_field X
                (slp_left_neumann_step tau c cutoff coefficient multiplier) z)))
          \<le> ereal ((1 / 2) * N)"
    using slp_nested_coefficient_product_left_esssup_contraction[
        ...
        next
          ...
          have successor_restrict:
              "?v = slp_restrict_field X
                (slp_left_neumann_step tau c cutoff coefficient ?u)"
          ...
          have contract_data:
              "(\<lambda>z :: slp_point.
                ereal (Real_Vector_Spaces.norm
                  (slp_restrict_field X
                    (slp_left_neumann_step tau c cutoff coefficient ?u) z)))
                \<in> borel_measurable (lborel :: slp_point measure)
              \<and>
              esssup (lborel :: slp_point measure)
                (\<lambda>z :: slp_point.
          ...
          have step_measurable:
            ...
            by (simp only: slp_left_neumann_step_def
