theory Inverse_Schrodinger_Lp_Nested_Coefficient_Product_High_Esssup
  imports
...
section \<open>Actual decaying above-two coefficient endpoint\<close>

context slp_cauchy_local_w1s
begin

theorem slp_nested_coefficient_product_high_esssup_bound:
...
proof -
  ...
  proof (rule exI[of _ ?C], rule conjI[OF C_positive])
    ...
    proof (intro allI impI)
      ...
      let ?u = "\<lambda>x. cutoff x * slp_dbar_psi_inverse tau c ?source x"
      let ?Du = "\<lambda>x. \<chi> i.
        ...
        slp_dbar_psi_inverse tau c ?source x *
          slp_complex_partial_derivative cutoff i x"
      let ?T = "slp_partial_psi_inverse tau c ?u"
