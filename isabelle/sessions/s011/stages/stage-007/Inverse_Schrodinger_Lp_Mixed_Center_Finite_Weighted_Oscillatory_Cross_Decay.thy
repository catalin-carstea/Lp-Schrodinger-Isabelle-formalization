theory Inverse_Schrodinger_Lp_Mixed_Center_Finite_Weighted_Oscillatory_Cross_Decay
  imports
...
section \<open>Signed one-terminal weighted oscillatory fiber decay\<close>

context aim_planar_riesz_hls_cauchy
begin

theorem slp_mixed_center_finite_weighted_oscillatory_cross_fiber_decay:
  fixes active_type ::
      ...
      "AE center in lborel.
        integrable lborel
          (slp_mixed_center_finite_weighted_active_complex_amplitude
            ...
            right_potential (\<lambda>_. 1) ?left_center center ::
            real^((unit + ((('i + 'i) + unit) + ('j + 'j))) \<times> bool)
              \<Rightarrow> complex)"
    by (rule
      slp_mixed_center_finite_weighted_active_amplitude_cross_integrable(1)[OF
        ...
        integrable lborel
          (slp_mixed_center_finite_weighted_active_complex_amplitude
            ...
            real^((unit + ((('i + 'i) + unit) + ('j + 'j))) \<times> bool)
              \<Rightarrow> complex)"
    by (rule
      slp_mixed_center_finite_weighted_active_amplitude_cross_integrable(2)[OF
        B_nonnegative C_nonnegative p_lower p_upper X_measurable X_bounded
        phi_test cutoff_measurable left_potential_lp right_potential_lp
        root_weight_lp root_weight_outside cutoff_bound root_support
        cutoff_support left_potential_support right_potential_support])
  show
    ...
    by (rule
      slp_mixed_center_finite_weighted_oscillatory_fiber_decay[OF
        ...
        left_potential_measurable left_terminal_measurable cutoff_measurable
        right_potential_measurable unit_measurable left_center_measurable
        left_amplitude_integrable])
  show
    "AE center in lborel.
      ((\<lambda>frequency.
        integral\<^sup>L lborel
    ...
    by (rule
      slp_mixed_center_finite_weighted_oscillatory_fiber_decay[OF
        stationary_phase density root_weight_measurable cutoff_measurable
        left_potential_measurable unit_measurable cutoff_measurable
        right_potential_measurable right_terminal_measurable
        right_center_measurable right_amplitude_integrable])
qed
