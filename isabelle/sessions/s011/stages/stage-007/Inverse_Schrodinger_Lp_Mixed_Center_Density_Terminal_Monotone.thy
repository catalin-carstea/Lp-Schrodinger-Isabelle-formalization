theory Inverse_Schrodinger_Lp_Mixed_Center_Density_Terminal_Monotone
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_007.Inverse_Schrodinger_Lp_Mixed_Center_Finite_Weighted_Fiber_Mass_Domination"
begin

section \<open>Terminal-weight monotonicity of positive mixed densities\<close>

theorem slp_positive_output_density_terminal_mono:
  assumes terminal_mono: "\<And>x. terminal_weight x \<le> terminal_majorant x"
  shows
...
proof (induction n arbitrary: origin target)
  ...
  show ?case
    ...
    by (rule mult_left_mono[OF terminal_mono], simp)
next
  case (Suc n)
  show ?case
    unfolding slp_positive_output_density.simps
...
theorem slp_left_positive_output_density_terminal_mono:
  assumes terminal_mono: "\<And>x. terminal_weight x \<le> terminal_majorant x"
  shows
  ...
  by (rule slp_positive_output_density_terminal_mono[OF terminal_mono])

theorem slp_right_positive_output_density_terminal_mono:
  assumes terminal_mono: "\<And>x. terminal_weight x \<le> terminal_majorant x"
  shows
...
theorem slp_mixed_center_density_terminal_mono:
  assumes left_terminal_mono:
      "\<And>x. left_terminal_weight x \<le> left_terminal_majorant x"
    and right_terminal_mono:
      "\<And>x. right_terminal_weight x \<le> right_terminal_majorant x"
  shows
...
proof -
  have left_mono:
    ...
    for root target
    by (rule slp_left_positive_output_density_terminal_mono[OF
          left_terminal_mono])
  have right_mono:
    ...
    for root target
    by (rule slp_right_positive_output_density_terminal_mono[OF
          right_terminal_mono])
  show ?thesis
