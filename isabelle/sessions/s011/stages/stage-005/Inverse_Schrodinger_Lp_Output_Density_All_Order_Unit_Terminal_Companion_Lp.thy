theory Inverse_Schrodinger_Lp_Output_Density_All_Order_Unit_Terminal_Companion_Lp
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_005.Inverse_Schrodinger_Lp_Mixed_Center_One_Terminal_Exponents"
begin

section \<open>All-order unit-terminal branch control at the mixed companion exponent\<close>

context aim_planar_riesz_hls
begin

theorem slp_left_right_positive_output_density_all_orders_unit_terminal_companion:
  fixes R C p :: real
      ...
      "\<And>n. \<exists>L. 0 \<le> L \<and>
        ...
        (\<forall>origin. integral\<^sup>L lborel
          (\<lambda>output. enn2real
            (slp_left_positive_output_density R cutoff potential (\<lambda>_. 1)
              n origin output) powr slp_mixed_unit_branch_exponent p) \<le> L)"
        ...
        (\<forall>origin. integral\<^sup>L lborel
          (\<lambda>output. enn2real
            (slp_right_positive_output_density R cutoff potential (\<lambda>_. 1)
              n origin output) powr slp_mixed_unit_branch_exponent p) \<le> L)"
proof -
  let ?a = "slp_mixed_unit_branch_exponent p"
  let ?b = "slp_mixed_unit_branch_holder_exponent p"
  ...
  have kernel_power_integrable:
    ...
    by (rule slp_localized_cauchy_kernel_power_integrable[OF
          less_imp_le[OF exponents(3)] exponents(4)])
  ...
  proof -
    ...
    have weighted_lp:
      ...
      by (rule slp_positive_ennreal_lp_bounded_multiplier[OF
            less_trans[OF zero_less_one exponents(3)] h_measurable
  ...
  proof -
    ...
    have kernel_integral:
      ...
      using slp_localized_cauchy_kernel_power_translate_integral[OF
          less_imp_le[OF exponents(3)] exponents(4), of R origin]
  ...
  have all_orders:
    ...
    by (rule slp_positive_output_density_all_orders_uniform_power[OF
          radius_nonnegative p_lower p_upper exponents(3) exponents(5)
          exponents(6) cutoff_measurable potential_lp terminal_measurable
