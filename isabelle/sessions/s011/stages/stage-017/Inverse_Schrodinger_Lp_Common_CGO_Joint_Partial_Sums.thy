theory Inverse_Schrodinger_Lp_Common_CGO_Joint_Partial_Sums
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_017.Inverse_Schrodinger_Lp_Joint_Geometric_Partial_Sum"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_017.Inverse_Schrodinger_Lp_CGO_Born_Finite_Expansion"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Literal joint Neumann prefixes from the common geometric data\<close>

theorem slp_left_neumann_joint_geometric_data_partial_sum_AE:
  ...
  shows
    "AE pair in lborel \<Otimes>\<^sub>M lborel.
      fst pair \<in> centers \<longrightarrow> (\<forall>N.
        norm (slp_left_neumann_partial_sum X N tau (fst pair)
...
proof -
  ...
  proof eventually_elim
    ...
    proof
      ...
      show
        "\<forall>N. norm (slp_left_neumann_partial_sum X N tau (fst pair)
          cutoff coefficient (snd pair)) \<le>
        (C * tau powr (-(1 - 1 / p) + epsilon) *
          (aim_complex_lp_norm p coefficient +
            norm (slp_dbar_inverse coefficient (fst pair)))) /
        ...
        show
          ...
          (C * tau powr (-(1 - 1 / p) + epsilon) *
            (aim_complex_lp_norm p coefficient +
              norm (slp_dbar_inverse coefficient (fst pair)))) /
          (1 - ?rho)"
          using prefixes[rule_format, of N] center_in
...
theorem slp_right_neumann_joint_geometric_data_partial_sum_AE:
  fixes p epsilon C CV T tau :: real
    and X centers :: "slp_point set"
    and cutoff coefficient :: slp_scalar_field
  assumes data:
  ...
  shows
    "AE pair in lborel \<Otimes>\<^sub>M lborel.
      fst pair \<in> centers \<longrightarrow> (\<forall>N.
        norm (slp_right_neumann_partial_sum X N tau (fst pair)
          cutoff coefficient (snd pair)) \<le>
        (C * tau powr (-(1 - 1 / p) + epsilon) *
          (aim_complex_lp_norm p coefficient +
...
proof -
  ...
  proof eventually_elim
    ...
    proof
      assume center_in: "fst pair \<in> centers"
      show
        "\<forall>N. norm (slp_right_neumann_partial_sum X N tau (fst pair)
          cutoff coefficient (snd pair)) \<le>
        (C * tau powr (-(1 - 1 / p) + epsilon) *
          (aim_complex_lp_norm p coefficient +
            norm (slp_partial_inverse coefficient (fst pair)))) /
