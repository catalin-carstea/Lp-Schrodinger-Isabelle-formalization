theory Inverse_Schrodinger_Lp_Right_Initial_Center_Value_Power_Loss
  imports
...
section \<open>Right initial center-value power-loss bound\<close>

context slp_cauchy_local_w1s
begin

theorem slp_right_initial_center_value_esssup_power_loss_bound:
    ...
    proof (intro allI impI)
      ...
      have output_measurable:
          "(\<lambda>z :: slp_point.
            ereal (Real_Vector_Spaces.norm
              (slp_restrict_field X
                (slp_dbar_psi_inverse (- tau) c ?u) z)))
            \<in> borel_measurable (lborel :: slp_point measure)"
