theory Inverse_Schrodinger_Lp_Left_Neumann_Joint_Geometric_Series
  imports
...
section \<open>Literal left Neumann series on a measurable center carrier\<close>

context slp_cauchy_local_w1s
begin

theorem slp_left_neumann_joint_geometric_series:
...
proof -
  ...
  have dbar_measurable:
      "slp_dbar_inverse coefficient \<in> borel_measurable lborel"
    by (rule slp_cauchy_transform_measurable_below_two[
          where orientation=SLP_Dbar_Inverse and p=p,
      ...
      proof -
        ...
        have totalized_iterate:
            "(\<lambda>pair :: slp_point \<times> slp_point.
              ...
              then slp_left_neumann_iterate j tau (fst pair) cutoff coefficient
                SLP_Dbar_Inverse (snd pair)
              else 0) \<in>
                borel_measurable (lborel \<Otimes>\<^sub>M lborel)"
          by (rule slp_left_neumann_iterate_conull_zero_joint_measurable[
        ...
        have abstract_iterate:
          ...
          by (simp only: UNIV_I if_True slp_left_neumann_iterate_eq_abstract)
        show
          "(\<lambda>pair :: slp_point \<times> slp_point.
            ?U j (fst pair) (snd pair)) \<in>
              borel_measurable (lborel \<Otimes>\<^sub>M lborel)"
