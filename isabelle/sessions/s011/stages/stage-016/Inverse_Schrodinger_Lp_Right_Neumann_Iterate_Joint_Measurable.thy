theory Inverse_Schrodinger_Lp_Right_Neumann_Iterate_Joint_Measurable
  imports
...
section \<open>Joint measurability of finite right Neumann iterates\<close>

theorem slp_right_neumann_base_joint_measurable:
  ...
  shows
    "(\<lambda>pair :: slp_point \<times> slp_point.
        slp_right_neumann_base tau (fst pair) cutoff potential orientation
          (snd pair)) \<in>
...
theorem slp_right_neumann_step_joint_measurable:
...
theorem slp_right_neumann_iterate_conull_zero_joint_measurable:
  assumes cutoff_measurable: "cutoff \<in> borel_measurable lborel"
    and potential_measurable: "potential \<in> borel_measurable lborel"
  ...
  shows
    "(\<lambda>pair :: slp_point \<times> slp_point.
        ...
        then slp_right_neumann_iterate n tau (fst pair) cutoff potential
...
proof -
  have iterate_joint:
      "(\<lambda>pair :: slp_point \<times> slp_point.
          slp_right_neumann_iterate n tau (fst pair) cutoff potential
