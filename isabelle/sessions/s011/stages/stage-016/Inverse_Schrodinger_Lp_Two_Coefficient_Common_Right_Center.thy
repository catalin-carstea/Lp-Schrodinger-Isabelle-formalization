theory Inverse_Schrodinger_Lp_Two_Coefficient_Common_Right_Center
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Smooth_Domain_Common_Carrier"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Neumann_AE_Center_Fixed_Point"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Common right fixed-point data for two coefficients\<close>

definition slp_right_neumann_center_fixed_point_data ::
  ...
  where
  "slp_right_neumann_center_fixed_point_data X T c cutoff coefficient \<longleftrightarrow>
    slp_cauchy_integrable_at SLP_Partial_Inverse coefficient c
          ...
          candidate z =
            slp_restrict_field X
              (slp_right_neumann_base tau c cutoff coefficient
                SLP_Partial_Inverse) z +
            slp_restrict_field X
              (slp_right_neumann_step tau c cutoff coefficient candidate) z)
        \<longrightarrow>
        (AE z in lborel.
          candidate z =
            slp_right_neumann_series_sum X tau c cutoff coefficient z)))"

theorem slp_right_neumann_center_fixed_point_data_mono:
  assumes data:
    "slp_right_neumann_center_fixed_point_data X T c cutoff coefficient"
    and threshold: "T \<le> T'"
  shows
    "slp_right_neumann_center_fixed_point_data X T' c cutoff coefficient"
proof -
  note unfolded_data =
    data[unfolded slp_right_neumann_center_fixed_point_data_def]
  note integrable = conjunct1[OF unfolded_data]
  note universal = conjunct2[OF unfolded_data]
  show ?thesis
    unfolding slp_right_neumann_center_fixed_point_data_def
    apply (intro conjI)
...
theorem slp_two_coefficient_common_right_center_fixed_point:
...
proof -
  obtain T1 :: real where T1_lower: "2 \<le> T1"
    ...
    using slp_right_neumann_series_ae_center_fixed_point_unique[
  ...
  proof eventually_elim
    ...
    proof
      ...
      show
        "slp_right_neumann_center_fixed_point_data
          X ?T c cutoff coefficient"
        by (rule slp_right_neumann_center_fixed_point_data_mono[OF old_data]) simp
    qed
  qed
  ...
  obtain Z where Z_measurable:
    ...
    using slp_bounded_smooth_domain_common_carrier[
      where P = "\<lambda>c. slp_right_neumann_center_fixed_point_data
        X ?T c cutoff coefficient"
        and Q = "\<lambda>c. slp_right_neumann_center_fixed_point_data
          X ?T c cutoff coefficient_tilde",
