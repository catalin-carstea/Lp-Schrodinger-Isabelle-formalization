theory Inverse_Schrodinger_Lp_Two_Coefficient_Common_Right_Joint_Series
  imports
    ...
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Common_Center_Carrier"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Common-carrier joint right series for two coefficients\<close>

definition slp_right_neumann_joint_geometric_data ::
...
theorem slp_right_neumann_joint_geometric_data_mono:
  assumes data:
    "slp_right_neumann_joint_geometric_data
...
theorem slp_two_coefficient_common_right_joint_series:
  ...
  shows
    "\<exists>T Z C CV C_tilde CV_tilde.
      ...
      \<and> (\<forall>tau c z. T \<le> tau \<and> c \<in> Z \<longrightarrow>
        slp_right_neumann_joint_series_on
...
proof -
  ...
  have joint_exists:
      "\<exists>C CV T1. slp_right_neumann_joint_geometric_data
  ...
  have joint_tilde_exists:
      "\<exists>C_tilde CV_tilde T2. slp_right_neumann_joint_geometric_data
  ...
  have joint_data_max:
      "slp_right_neumann_joint_geometric_data
        p epsilon X Z cutoff coefficient C CV ?T"
    by (rule slp_right_neumann_joint_geometric_data_mono[
          OF joint_data T1_le])
      ...
      "\<forall>tau c z. ?T \<le> tau \<and> c \<in> Z \<longrightarrow>
        ...
        \<and> slp_right_neumann_joint_series_on
          Z X tau cutoff coefficient_tilde (c, z) =
            slp_right_neumann_series_sum X tau c cutoff coefficient_tilde z"
    by (intro allI impI)
      (simp add: slp_right_neumann_joint_series_on_fiber)
