theory Inverse_Schrodinger_Lp_Right_Initial_Correction_Power_Loss
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Initial_Cauchy_Source_Power_Loss"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Initial_Center_Value_Power_Loss"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_014.Inverse_Schrodinger_Lp_Exact_Neumann_Iterate_Bridge"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Literal right initial-correction power-loss bound\<close>

context slp_cauchy_local_w1s
begin

theorem slp_right_neumann_base_esssup_power_loss_bound:
...
proof -
          ...
          ereal (Real_Vector_Spaces.norm
            (slp_restrict_field X
              (slp_dbar_psi_inverse (- tau) c
                (\<lambda>x. cutoff x *
                  slp_partial_inverse coefficient x)) z)))
          \<in> borel_measurable (lborel :: slp_point measure)
        \<and>
        esssup (lborel :: slp_point measure)
          (\<lambda>z :: slp_point.
          ...
          \<le> ereal
            (Cq * tau powr (-(1 - 1 / p) + epsilon) *
              aim_complex_lp_norm p coefficient)"
    using slp_right_initial_cauchy_source_esssup_power_loss_bound[
  ...
  obtain Cc :: real where Cc_positive: "0 < Cc"
    ...
    using slp_right_initial_center_value_esssup_power_loss_bound[
    ...
    proof (intro allI impI)
      ...
      assume data:
        "2 \<le> tau \<and>
          ...
          aim_complex_lp_on_plane p coefficient \<and>
          {x. coefficient x \<noteq> 0} \<subseteq> Y"
      let ?primitive = "slp_partial_inverse coefficient"
      let ?first =
        "slp_dbar_psi_inverse (- tau) c
      ...
      proof -
        ...
        show ?thesis
          using raw unfolding slp_dbar_psi_inverse_def zero_modulation
          by simp
      qed
      ...
      proof -
        ...
        have literal_integrable:
            "slp_cauchy_integrable_at SLP_Dbar_Inverse ?literal z"
          unfolding literal_eq by (rule difference_integrable)
        have congruent:
            "slp_cauchy_transform SLP_Dbar_Inverse ?literal z =
              slp_cauchy_transform SLP_Dbar_Inverse ?difference z"
        ...
        have additive:
            "slp_cauchy_transform SLP_Dbar_Inverse ?difference z =
              slp_cauchy_transform SLP_Dbar_Inverse ?u z +
                slp_cauchy_transform SLP_Dbar_Inverse ?negative_v z"
          by (rule slp_cauchy_transform_add[
                OF first_integrable negative_integrable])
        have negative:
            "slp_cauchy_transform SLP_Dbar_Inverse ?negative_v z =
              (- 1) * slp_cauchy_transform SLP_Dbar_Inverse ?v z"
          by (rule slp_cauchy_transform_mult_left[OF second_integrable_at])
        show ?thesis
          using congruent additive negative
          unfolding slp_right_neumann_base_def
            slp_dbar_psi_inverse_def by simp
      qed
      have restricted_eq:
