theory Inverse_Schrodinger_Lp_Right_Neumann_Iterate_Lpstar_Geometric
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Initial_Correction_Lpstar"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_016.Inverse_Schrodinger_Lp_Right_Neumann_Iterate_Geometric_Contraction"
begin

hide_const (open) Commutative_Ring.norm

section \<open>Literal right Neumann iterates in both manuscript norms\<close>

context slp_cauchy_local_w1s
begin

theorem slp_right_neumann_iterate_lpstar_geometric:
...
proof -
          ...
          \<le> ereal
            (C0 * tau powr (-(1 - 1 / p) + epsilon) *
              (aim_complex_lp_norm p coefficient +
                Real_Vector_Spaces.norm
                  (slp_partial_inverse coefficient c)))"
    using slp_right_neumann_base_esssup_power_loss_bound[
        ...
        slp_complex_lp_norm_on (aim_hls_target_exponent p) X
          ...
          \<le> CB * inverse (sqrt tau) *
            (aim_complex_lp_norm p coefficient +
              Real_Vector_Spaces.norm
                (slp_partial_inverse coefficient c))"
    using slp_right_neumann_base_lpstar_inverse_sqrt_bound[
      OF riesz_hls cauchy_test_left_inverse evans_density exponent_lower
        ...
        esssup (lborel :: slp_point measure)
          ...
          \<le> ereal
            (CS * M * tau powr (-(1 - 1 / p) + epsilon) *
              aim_complex_lp_norm p coefficient)"
    using slp_nested_coefficient_product_right_esssup_power_loss_bound[
      OF riesz_hls cauchy_test_left_inverse evans_density exponent_lower
        exponent_upper loss_positive radius_nonnegative radius_lower set_radius
        X_open X_bounded X_bounded cutoff_test cutoff_bound
        cutoff_derivative_zero_bound cutoff_derivative_one_bound
  ...
  proof (rule exI[of _ ?C], rule exI[of _ ?CV], rule exI[of _ ?T])
    ...
    next
      ...
      let ?rho = "?CV * ?rate"
      let ?B =
        "?Q + Real_Vector_Spaces.norm (slp_partial_inverse coefficient c)"
      let ?M0 = "C0 * ?rate * ?B"
      let ?step = "slp_right_neumann_step tau c cutoff coefficient"
      let ?base =
