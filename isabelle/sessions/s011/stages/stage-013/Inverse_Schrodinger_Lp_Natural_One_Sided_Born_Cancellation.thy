theory Inverse_Schrodinger_Lp_Natural_One_Sided_Born_Cancellation
  imports
...
section \<open>All-natural one-sided Born cancellation\<close>

theorem slp_natural_one_sided_bracket_integrand_decomposition:
  ...
  shows "let P = PiM {..<n} (\<lambda>_::nat. (lborel :: slp_point measure));
             ...
             W = (\<lambda>T H. slp_natural_one_sided_weighted_amplitude
               n Q cutoff q T H);
...
theorem slp_left_born_functional_natural_cancellation:
  ...
  shows "((\<lambda>tau. slp_left_born_functional n tau phi Q cutoff q orientation)
...
proof -
  ...
  let ?s = "\<lambda>z. snd (fst z)"
  let ?output = "\<lambda>z. slp_left_branch_output (?ps z) (?s z)"
  let ?residual = "\<lambda>z. slp_left_branch_residual (?ps z) (?s z)"
  let ?one = "\<lambda>_::slp_point. (1::complex)"
  let ?A = "slp_cauchy_transform orientation q"
  let ?W = "\<lambda>T H. slp_natural_one_sided_weighted_amplitude n Q cutoff q T H"
  let ?a = "?W ?one ?one"
  ...
  qed
  note principal = one_hls.slp_natural_one_sided_principal_decay[
  ...
  have F0_integrable: "integrable ?MJ (?F0 tau)" for tau
    using one_hls.slp_natural_one_sided_principal_components_integrable[
      ...
      where n=n and orientation=orientation and omega=tau]
    by (auto simp only: Let_def)
  note smooth = one_hls.slp_natural_one_sided_smooth_error_decay[
  ...
  have smooth_limit: "(?J1 \<longlongrightarrow> 0) at_top"
    using smooth by (auto simp only: Let_def)
  note cauchy = one_hls.slp_natural_one_sided_cauchy_error_decay[
    where n=n and R=R and C=C and p=p and X=Y and cutoff=cutoff and q=q
      and Q=Q and phi=phi and orientation=orientation
      and psi="\<lambda>tau z. tau * ?residual z",
    OF R_nonnegative p_lower p_upper Y_measurable Y_bounded
  ...
  have pointwise:
    ...
    using slp_natural_one_sided_bracket_integrand_decomposition[
  ...
  have born_identity:
    "slp_left_born_functional n tau phi Q cutoff q orientation = ?full tau"
    for tau
    using one_hls.slp_natural_one_sided_born_bracket_identity[
      OF R_nonnegative p_lower p_upper Y_measurable Y_bounded
        cutoff_measurable q_lp Q_lp Q_outside cutoff_bound C_nonnegative
        Q_support cutoff_support q_support phi_test,
