theory Inverse_Schrodinger_Lp_Natural_One_Sided_Smooth_Error_Decay
  imports
...
section \<open>All-natural one-sided smooth averaging error\<close>

context aim_planar_riesz_hls_cauchy
begin

theorem slp_natural_one_sided_smooth_error_decay:
  ...
  shows "eventually (\<lambda>tau. integrable
      ...
      (\<lambda>z. exp (\<i> * of_real (psi tau z)) *
        slp_natural_one_sided_weighted_amplitude n Q cutoff q
...
proof -
  ...
  show "eventually (\<lambda>tau. integrable ?MJ
      (\<lambda>z. exp (\<i> * of_real (psi tau z)) *
        slp_natural_one_sided_weighted_amplitude n Q cutoff q ?A (?E tau) z)) at_top"
    by (rule transfer(1))
  show "((\<lambda>tau. integral\<^sup>L ?MJ
      (\<lambda>z. exp (\<i> * of_real (psi tau z)) *
        slp_natural_one_sided_weighted_amplitude n Q cutoff q ?A (?E tau) z))
      \<longlongrightarrow> 0) at_top"
    by (rule transfer(2))
qed
