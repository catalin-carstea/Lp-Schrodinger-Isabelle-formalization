theory Inverse_Schrodinger_Lp_Left_Graph_Natural_Coordinate_Integrable
  imports
...
section \<open>Term-indexed natural-coordinate graph integrability\<close>

definition slp_left_branch_natural_value ::
    "nat \<Rightarrow> (nat \<Rightarrow> slp_point) \<Rightarrow> nat \<Rightarrow> slp_point"
where
  "slp_left_branch_natural_value n family k =
    (if k < n then family k else 0)"

definition slp_left_branch_oscillatory_graph_kernel_natural ::
...
theorem slp_left_branch_oscillatory_graph_kernel_natural_integrable_hls:
...
proof -
  ...
  have raw_measurable[measurable]:
      "(\<lambda>coordinates.
        slp_left_branch_complex_kernel_list cutoff potential (\<lambda>_. 1)
          (map (\<lambda>k. (?pos k coordinates, ?neg k coordinates)) [0..<n])
  ...
  have kernel_measurable:
      "slp_left_branch_oscillatory_graph_kernel_natural n tau center cutoff
        potential (\<lambda>_. 1) origin \<in> borel_measurable ?M"
      ...
      "\<And>coordinates.
        ennreal (norm
          (slp_left_branch_oscillatory_graph_kernel_natural n tau center
            cutoff potential (\<lambda>_. 1) origin coordinates)) \<le>
  ...
  proof -
    ...
    show
      "ennreal (norm
        (slp_left_branch_oscillatory_graph_kernel_natural n tau center cutoff
          potential (\<lambda>_. 1) origin coordinates)) \<le>
        slp_left_branch_positive_kernel_list (2 * B) cutoff potential
          (\<lambda>_. 1) ?pairs origin (snd coordinates)"
    proof (cases
        "slp_left_branch_complex_kernel_list cutoff potential (\<lambda>_. 1)
  ...
  have positive_mass:
      "nn_integral ?M
        ...
        slp_positive_branch_functional (2 * B) cutoff potential
          (\<lambda>_. 1) n origin (\<lambda>_. 1)"
  proof -
    note functional =
      slp_left_branch_positive_kernel_list_integral_functional_distinct[
        ...
        OF distinct_upt cutoff_measurable potential_measurable
          unit_measurable unit_measurable]
    show ?thesis
  ...
  have functional_finite:
      "slp_positive_branch_functional (2 * B) cutoff potential (\<lambda>_. 1)
    ...
    using slp_positive_branch_mass_unweighted_finite[
      where R = "2 * B" and C = C and p = p and cutoff = cutoff
        and potential = potential and n = n and origin = origin,
  ...
  proof (intro conjI)
    ...
    show
      "nn_integral ?M (\<lambda>coordinates. ennreal (norm
        (slp_left_branch_oscillatory_graph_kernel_natural n tau center cutoff
          potential (\<lambda>_. 1) origin coordinates))) < \<infinity>"
