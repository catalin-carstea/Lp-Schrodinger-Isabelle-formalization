theory Inverse_Schrodinger_Lp_Mixed_Center_Finite_Positive_Fiber_Mass_Measurable
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_006.Inverse_Schrodinger_Lp_Mixed_Center_Finite_Positive_Mass_Finite"
begin

section \<open>Measurability of the finite mixed positive fiber mass\<close>

theorem slp_mixed_center_finite_positive_fiber_mass_measurable:
  ...
  shows
    "(\<lambda>center.
        slp_mixed_center_finite_positive_fiber_mass TYPE('i::finite)
          ...
          right_cutoff right_potential center) \<in> borel_measurable lborel"
proof -
  let ?integrand =
  ...
  have left_kernel_composed[measurable]:
      "((\<lambda>z. slp_left_branch_positive_kernel_joint R left_cutoff
          ...
          (fst (slp_mixed_center_finite_inserted_coordinates
            (fst z) (snd z)))) ::
          (slp_point \<times> ('i, 'j) slp_mixed_center_finite_coordinates)
            \<Rightarrow> ennreal) \<in> borel_measurable lborel"
