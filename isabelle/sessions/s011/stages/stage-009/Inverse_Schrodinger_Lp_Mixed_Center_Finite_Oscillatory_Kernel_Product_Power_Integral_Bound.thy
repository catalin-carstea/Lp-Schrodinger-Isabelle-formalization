theory Inverse_Schrodinger_Lp_Mixed_Center_Finite_Oscillatory_Kernel_Product_Power_Integral_Bound
  imports
    ...
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_009.Inverse_Schrodinger_Lp_Complex_Kernel_Lp_Power_Integral_Domination"
begin

section \<open>Frequency-uniform power bound for the literal mixed kernel\<close>

context aim_planar_riesz_hls
begin

theorem slp_mixed_center_finite_oscillatory_kernel_product_power_integral_bound:
      ...
      (\<lambda>center.
        norm_class.norm
          (slp_mixed_center_finite_oscillatory_kernel TYPE('i::finite)
            ...
            cutoff right_potential center)
          powr slp_mixed_product_dual_exponent p) \<le>
      integral\<^sup>L lborel
        (\<lambda>center.
          enn2real
            (slp_mixed_center_density (2 * B) cutoff left_potential
              ...
              root_weight center)
            powr slp_mixed_product_dual_exponent p)"
proof -
  ...
  note quantitative_domination =
    slp_complex_kernel_power_integral_le_positive_density[
      where s = ?dual and density = ?density and kernel = ?kernel,
      OF dual_positive density_lp kernel_measurable kernel_dominated]
  show ?thesis
    by (rule quantitative_domination(2))
qed
