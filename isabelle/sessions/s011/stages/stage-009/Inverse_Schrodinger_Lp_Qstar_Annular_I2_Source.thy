theory Inverse_Schrodinger_Lp_Qstar_Annular_I2_Source
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_009.Inverse_Schrodinger_Lp_Qstar_Annular_I1_Lp_Bound"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_008.Inverse_Schrodinger_Lp_Quantitative_Complex_Holder"
begin

hide_const (open) Commutative_Ring.norm

section \<open>The scale-invariant source for the second annular term\<close>

definition slp_qstar_annular_I2_coefficient ::
  "real \<Rightarrow> slp_point \<Rightarrow> complex"
where
  "slp_qstar_annular_I2_coefficient delta y =
    (if delta \<le> norm y \<and> norm y \<le> 2 * delta then
      of_real (slp_radial_inverse y)
    else 0)"

definition slp_qstar_annular_I2_source ::
  "real \<Rightarrow> slp_scalar_field \<Rightarrow> slp_scalar_field"
where
  "slp_qstar_annular_I2_source delta f y =
    slp_qstar_annular_I2_coefficient delta y * f y"

lemma slp_qstar_annular_I2_coefficient_measurable[measurable]:
  "slp_qstar_annular_I2_coefficient delta
    \<in> borel_measurable lborel"
  unfolding slp_qstar_annular_I2_coefficient_def by measurable

lemma slp_qstar_annular_I2_coefficient_norm_square:
  "norm_class.norm (slp_qstar_annular_I2_coefficient delta y) powr 2 =
    slp_squared_radial_annulus delta (2 * delta) y"
  unfolding slp_qstar_annular_I2_coefficient_def
    slp_squared_radial_annulus_def slp_radial_inverse_square_def
  by (simp add: powr_realpow power2_eq_square)

theorem slp_qstar_annular_I2_source_bound:
  ...
  shows coefficient_lp:
      "aim_complex_lp_on_plane 2
        (slp_qstar_annular_I2_coefficient delta)"
    and coefficient_mass:
      "integral\<^sup>L lborel
          (\<lambda>y. norm_class.norm
            (slp_qstar_annular_I2_coefficient delta y) powr 2) =
    ...
    and source_power_bound:
      "integral\<^sup>L lborel
          (\<lambda>y. norm_class.norm
            (slp_qstar_annular_I2_source delta f y) powr a) \<le>
        (integral\<^sup>L lborel
          (slp_squared_radial_annulus 1 2)) powr (a / 2) *
        (integral\<^sup>L lborel
          (\<lambda>y. norm_class.norm (f y) powr
            aim_hls_target_exponent a)) powr
              (a / aim_hls_target_exponent a)"
    and source_norm_bound:
      "aim_complex_lp_norm a (slp_qstar_annular_I2_source delta f) \<le>
        (integral\<^sup>L lborel
          (slp_squared_radial_annulus 1 2)) powr (1 / 2) *
        aim_complex_lp_norm (aim_hls_target_exponent a) f"
proof -
  let ?q = "aim_hls_target_exponent a"
  let ?coefficient = "slp_qstar_annular_I2_coefficient delta"
  let ?source = "slp_qstar_annular_I2_source delta f"
  let ?K = "integral\<^sup>L lborel (slp_squared_radial_annulus 1 2)"
  ...
  have coefficient_power_eq:
      "(\<lambda>y. norm_class.norm (?coefficient y) powr 2) =
        slp_squared_radial_annulus delta (2 * delta)"
    by (rule ext, rule slp_qstar_annular_I2_coefficient_norm_square)
  ...
  have coefficient_mass_exact:
      "integral\<^sup>L lborel
          (\<lambda>y. norm_class.norm (?coefficient y) powr 2) = ?K"
  proof -
    have normalized:
        "integral\<^sup>L lborel
            (slp_squared_radial_annulus delta (2 * delta)) = ?K"
      using slp_squared_radial_annulus_integral_normalize[
          OF delta_positive, of "2 * delta"]
      by (simp add: delta_nonzero)
    show ?thesis
      unfolding coefficient_power_eq by (rule normalized)
  qed
  ...
  show coefficient_mass:
      "integral\<^sup>L lborel
          (\<lambda>y. norm_class.norm (?coefficient y) powr 2) = ?K"
    by (rule coefficient_mass_exact)
  ...
  have holder_bound:
      "integral\<^sup>L lborel
        ...
        ?K powr (a / 2) * ?F powr (a / ?q)"
  proof -
    note raw = holder(2)
    show ?thesis
      using raw coefficient_mass_exact a_nonzero q_nonzero
