theory Inverse_Schrodinger_Lp_Mixed_Center_Left_Cauchy_Right_Unit_Error_Decay
  imports
...
section \<open>The first literal one-factor mixed center error\<close>

context aim_planar_riesz_hls_cauchy
begin

theorem slp_mixed_center_finite_left_cauchy_right_unit_error_integral_decay:
  fixes B C p :: real
    and X :: "slp_point set"
...
proof -
  ...
  let ?source = "\<lambda>x. phi x * ?right_terminal x"
  let ?actual_error =
  ...
  let ?density =
    "slp_mixed_center_density (2 * B) cutoff left_potential right_potential
      (\<lambda>x. ennreal (cmod (?left_terminal x))) (\<lambda>_. 1)
  ...
  let ?kernel =
    "\<lambda>tau.
      slp_mixed_center_finite_weighted_oscillatory_kernel
        ...
        ?left_terminal cutoff right_potential (\<lambda>_. 1) (?aux_error tau)"
  let ?actual_kernel =
    "\<lambda>tau.
      slp_mixed_center_finite_weighted_oscillatory_kernel
        TYPE('i) TYPE('j) tau root_weight cutoff left_potential
        ?left_terminal cutoff right_potential (\<lambda>_. 1)
        (?actual_error tau)"
  ...
  have mass_finite:
      "nn_integral lborel
        (slp_mixed_center_finite_weighted_absolute_fiber_mass
          ...
          cutoff right_potential (\<lambda>_. 1) (?aux_error tau)) <
  ...
  have amplitude_integrable:
      "AE center in lborel.
        integrable lborel
          (slp_mixed_center_finite_weighted_complex_amplitude root_weight
            ...
            (\<lambda>_. 1) (?aux_error tau) center ::
    ...
    by (rule
      slp_mixed_center_finite_weighted_amplitude_fiber_integrable_from_mass[OF
        ...
        unit_measurable aux_error_measurable mass_finite])
  have kernel_measurable:
    ...
    by (rule
      slp_mixed_center_finite_weighted_oscillatory_kernel_properties(1)[OF
        root_weight_measurable cutoff_measurable left_potential_measurable
        left_terminal_measurable cutoff_measurable right_potential_measurable
        unit_measurable aux_error_measurable amplitude_integrable])
  have kernel_absolute_bound:
      "AE center in lborel.
        ennreal (norm (?kernel tau center)) \<le>
        slp_mixed_center_finite_weighted_absolute_fiber_mass
          TYPE('i) TYPE('j) root_weight cutoff left_potential ?left_terminal
          cutoff right_potential (\<lambda>_. 1) (?aux_error tau) center"
    for tau
    by (rule
  ...
  have fiber_mass_bound:
      "slp_mixed_center_finite_weighted_absolute_fiber_mass
          TYPE('i) TYPE('j) root_weight cutoff left_potential ?left_terminal
          cutoff right_potential (\<lambda>_. 1) (?aux_error tau) center \<le>
        ?majorant tau center"
    for tau center
  proof -
    have raw_native:
      ...
      by (rule
        slp_mixed_center_finite_weighted_absolute_fiber_mass_density_bound[OF
          ...
          right_potential_measurable unit_measurable root_support
