theory Inverse_Schrodinger_Lp_Fixed_One_Sided_Cancellations_Hormander
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_008.Inverse_Schrodinger_Lp_Left_Born_Finite_Decay_Hormander"
    "Paper_Inverse_Schrodinger_Lp_Proof_Stage_008.Inverse_Schrodinger_Lp_Right_Born_Finite_Decay_Hormander"
begin

section \<open>Fixed one-sided Born cancellations from the physical strong L2 theory\<close>

context aim_planar_riesz_hls_cauchy
begin

theorem slp_fixed_one_sided_born_cancellations_hormander:
  ...
  show
    "((\<lambda>tau.
        slp_left_born_functional CARD('i) tau phi left_potential cutoff
          left_potential left_orientation)
      \<longlongrightarrow> 0) at_top"
    by (rule slp_left_born_functional_finite_decay_hormander[
          where B = B and C = C and p = p and X = X,
          OF fourier_plancherel stationary_left density_left B_nonnegative
            p_lower p_upper X_measurable X_bounded left_potential_lp
            left_potential_outside cutoff_measurable cutoff_bound C_nonnegative
            cutoff_support left_potential_support phi_test uniform_convergence])
  show
    "((\<lambda>tau.
        slp_right_born_functional CARD('j) tau phi right_potential cutoff
          right_potential right_orientation)
      \<longlongrightarrow> 0) at_top"
    by (rule slp_right_born_functional_finite_decay_hormander[
          where B = B and C = C and p = p and X = X,
          OF fourier_plancherel stationary_right density_right B_nonnegative
            p_lower p_upper X_measurable X_bounded right_potential_lp
            right_potential_outside cutoff_measurable cutoff_bound C_nonnegative
            cutoff_support right_potential_support phi_test uniform_convergence])
qed
