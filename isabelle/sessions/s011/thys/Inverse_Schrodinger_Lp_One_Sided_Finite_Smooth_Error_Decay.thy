theory Inverse_Schrodinger_Lp_One_Sided_Finite_Smooth_Error_Decay
  imports
    "Paper_Inverse_Schrodinger_Lp_Proof.Inverse_Schrodinger_Lp_One_Sided_Finite_Oscillatory_Terminal_Bound"
    "Paper_Inverse_Schrodinger_Lp_Proof.Inverse_Schrodinger_Lp_Terminal_Weighted_Center_Average_Error"
begin

section \<open>Finite one-sided smooth-error decay\<close>

theorem slp_positive_root_output_density_pairing_cutoff_mono:
  assumes cutoff_measurable[measurable]:
    ...
    and terminal_majorant_measurable[measurable]:
    ...
    and terminal_weight_le:
      "\<And>x. ennreal (Real_Vector_Spaces.norm (cutoff x)) *
          terminal_weight x \<le>
        ennreal (Real_Vector_Spaces.norm (cutoff x)) * terminal_majorant x"
  shows
    "(\<integral>\<^sup>+ output.
        slp_positive_root_output_density R cutoff potential terminal_weight n
          root_weight output * test output
        \<partial>lborel) \<le>
      (\<integral>\<^sup>+ output.
        slp_positive_root_output_density R cutoff potential
          terminal_majorant n root_weight output * test output
        \<partial>lborel)"
proof -
  have actual_pairing:
    ...
    by (rule slp_positive_root_output_density_pairing[OF
          ...
          root_weight_measurable test_measurable])
  have majorant_pairing:
      "(\<integral>\<^sup>+ output. test output *
          slp_positive_root_output_density R cutoff potential
            terminal_majorant n root_weight output
          \<partial>lborel) =
        (\<integral>\<^sup>+ root. ennreal (Real_Vector_Spaces.norm
            (root_weight root)) *
          slp_positive_branch_functional R cutoff potential
            terminal_majorant n root test
          \<partial>lborel)"
    by (rule slp_positive_root_output_density_pairing[OF
          cutoff_measurable potential_measurable terminal_majorant_measurable
          root_weight_measurable test_measurable])
  have functional_le:
      "slp_positive_branch_functional R cutoff potential terminal_weight n
          root test \<le>
        slp_positive_branch_functional R cutoff potential terminal_majorant n
          root test"
    for root
    by (rule slp_positive_branch_functional_cutoff_mono[OF
          terminal_weight_le]) simp
  have root_integral_le:
      "(\<integral>\<^sup>+ root. ennreal (Real_Vector_Spaces.norm
          (root_weight root)) *
        slp_positive_branch_functional R cutoff potential terminal_weight n
          root test
  ...
  proof (rule nn_integral_mono)
    fix root
    show "ennreal (Real_Vector_Spaces.norm (root_weight root)) *
          slp_positive_branch_functional R cutoff potential terminal_weight n
            root test \<le>
        ennreal (Real_Vector_Spaces.norm (root_weight root)) *
          slp_positive_branch_functional R cutoff potential terminal_majorant n
            root test"
      by (rule mult_left_mono[OF functional_le]) simp
  qed
  show ?thesis
    using actual_pairing majorant_pairing root_integral_le
...
theorem slp_left_branch_finite_oscillatory_center_average_error_decay:
  ...
  proof eventually_elim
    ...
    proof -
      have terminal_bound:
          "ennreal (Real_Vector_Spaces.norm (?I tau)) \<le>
            (\<integral>\<^sup>+ output.
              ...
              \<partial>lborel)"
      proof (rule slp_left_branch_finite_oscillatory_integral_terminal_bound[
      ...
      show ?thesis
        ...
        unfolding slp_left_one_sided_output_density_def by simp
    qed
    show ?case
