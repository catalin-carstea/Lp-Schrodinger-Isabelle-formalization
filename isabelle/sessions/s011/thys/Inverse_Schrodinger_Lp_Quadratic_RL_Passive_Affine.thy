theory Inverse_Schrodinger_Lp_Quadratic_RL_Passive_Affine
  imports
    Inverse_Schrodinger_Lp_Quadratic_RL_Passive
    Inverse_Schrodinger_Lp_Quadratic_RL_Affine
begin

section \<open>Passive-dependent affine quadratic decay\<close>

lemma slp_product_affine_quadratic_phase_integrable:
  fixes A :: "real^'n::finite^'n"
    and b :: "real^'c::finite \<Rightarrow> real^'n"
    and p0 :: "real^'c \<Rightarrow> real"
    and F :: "real^'c \<Rightarrow> real^'n \<Rightarrow> complex"
    and omega :: real
  assumes b_measurable: "b \<in> borel_measurable lborel"
...
proof (rule Bochner_Integration.integrable_bound[OF F_integrable])
  ...
  have quadratic_measurable:
    ...
    using borel_measurable_continuous_onI[OF quadratic_half_continuous]
    by simp
...
lemma slp_passive_affine_quadratic_decay:
  fixes A :: "real^'n::finite^'n"
    and b :: "real^'c::finite \<Rightarrow> real^'n"
    and p0 :: "real^'c \<Rightarrow> real"
    and F :: "real^'c \<Rightarrow> real^'n \<Rightarrow> complex"
...
proof -
  ...
  have norm_product_integrable:
    ...
    using Bochner_Integration.integrable_norm[OF F_integrable] by blast
  ...
  have product_integrable:
    "integrable (lborel \<Otimes>\<^sub>M lborel)
      (\<lambda>(c, u). exp (\<i> * of_real
        (omega * (inner u (A *v u) / 2 +
          inner (b c) u + p0 c))) * F c u)" for omega
    by (rule slp_product_affine_quadratic_phase_integrable[
      OF b_measurable p0_measurable F_integrable])
  have slice_integrable: "integrable lborel (?slice omega)" for omega
    using lborel_pair.integrable_fst[OF product_integrable[of omega]] by simp
  ...
  have product_as_slices:
    ...
    using lborel_pair.integral_fst[OF product_integrable[of omega]] by simp
