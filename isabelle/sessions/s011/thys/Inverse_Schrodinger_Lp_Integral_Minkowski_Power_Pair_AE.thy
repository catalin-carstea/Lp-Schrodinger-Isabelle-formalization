theory Inverse_Schrodinger_Lp_Integral_Minkowski_Power_Pair_AE
  imports Inverse_Schrodinger_Lp_Weighted_Joint_Integrable_Pair
begin

section \<open>Integral Minkowski with almost-everywhere pair fibers\<close>

lemma slp_integral_minkowski_power_pair_AE:
  ...
  assumes a_lower: "1 < a"
    ...
    and fiber_power_integrable:
      "AE u in lborel.
        integrable lborel (\<lambda>t. weight t * datum t u powr a)"
...
proof -
  ...
  have pointwise_bound:
      "AE u in lborel. ?H u powr a \<le> ?W powr (a / b) * ?P u"
  ...
  proof (rule Bochner_Integration.integrable_bound[OF majorant_integrable
    ...
    proof eventually_elim
      fix u :: slp_point
      assume bound: "?H u powr a \<le> ?W powr (a / b) * ?P u"
      show "norm (?H u powr a) \<le>
          norm (?W powr (a / b) * ?P u)"
        using bound H_nonnegative[of u] P_nonnegative[of u] W_nonnegative
        by simp
    qed
  qed
  ...
  proof -
    have integral_bound:
      ...
      by (rule integral_mono_AE[OF target_integrable majorant_integrable
            pointwise_bound])
    show ?thesis
